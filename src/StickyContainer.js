...
export default class StickyContainer extends Component {
            ...
            <Context.Consumer>
                {
                    context => {
                        ...
                        return (
                            <Context.Provider value={{ stickyScrollY, stickyManager, scrollView, stickyContainerKey: this.key }}>
                                <View
    ...
    _onLayout(event) {
        if (!this.firstLoad && Math.abs(this.height - event.nativeEvent.layout.height) > 5) {
